-- Secure RLS Setup for HostTrack
-- This script creates RLS policies that work with existing data
...
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'analytics') THEN
        ...
        -- Drop existing policies
        EXECUTE 'DROP POLICY IF EXISTS "analytics_select_policy" ON analytics';
        EXECUTE 'DROP POLICY IF EXISTS "analytics_insert_policy" ON analytics';
        EXECUTE 'DROP POLICY IF EXISTS "analytics_update_policy" ON analytics';
        EXECUTE 'DROP POLICY IF EXISTS "analytics_delete_policy" ON analytics';
        
        -- Create policies
        EXECUTE 'CREATE POLICY "analytics_select_policy" ON analytics FOR SELECT USING (auth.uid() = owner_id)';
        EXECUTE 'CREATE POLICY "analytics_insert_policy" ON analytics FOR INSERT WITH CHECK (auth.uid() = owner_id)';
        EXECUTE 'CREATE POLICY "analytics_update_policy" ON analytics FOR UPDATE USING (auth.uid() = owner_id)';
        EXECUTE 'CREATE POLICY "analytics_delete_policy" ON analytics FOR DELETE USING (auth.uid() = owner_id)';
    END IF;
END $$; 