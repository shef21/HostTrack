...
# Observatory coordinates (centered around Lower Main Road)
ne_lat = -33.9350  # North-East latitude
ne_long = 18.4750  # North-East longitude
sw_lat = -33.9450  # South-West latitude
sw_long = 18.4650  # South-West longitude
zoom_value = 15  # Increased zoom level for more precise area focus

try:
    # Search listings
    print("Searching for listings in Observatory (Lower Main Road area)...")
    ...
    # Save raw data
    with open('airbnb_observatory_limited.json', 'w', encoding='utf-8') as f:
        json.dump(limited_results, f, indent=2, ensure_ascii=False)
    print("Raw results saved to airbnb_observatory_limited.json")
