-- Step 1: Clean up any leftover objects from previous attempts
DROP TYPE IF EXISTS cape_town_area_v2 CASCADE;
ALTER TABLE IF EXISTS cape_town_competitors DROP COLUMN IF EXISTS area_new;
ALTER TABLE IF EXISTS cape_town_market_data DROP COLUMN IF EXISTS area_new;
ALTER TABLE IF EXISTS cape_town_market_trends DROP COLUMN IF EXISTS area_new;
ALTER TABLE IF EXISTS cape_town_area_stats DROP COLUMN IF EXISTS area_new;
...
-- Step 3: Add new column with new enum type to each table
...
UPDATE cape_town_competitors SET area_new = area::text::cape_town_area_v2 WHERE area IS NOT NULL;
UPDATE cape_town_market_data SET area_new = area::text::cape_town_area_v2 WHERE area IS NOT NULL;
UPDATE cape_town_market_trends SET area_new = area::text::cape_town_area_v2 WHERE area IS NOT NULL;
UPDATE cape_town_area_stats SET area_new = area::text::cape_town_area_v2 WHERE area IS NOT NULL;
...
-- Step 9: Add NOT NULL constraints back
