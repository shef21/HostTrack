...
ne_lat = -33.9000  # North-East latitude
ne_long = 18.4300  # North-East longitude
sw_lat = -33.9200  # South-West latitude
sw_long = 18.4100  # South-West longitude
zoom_value = 14  # Zoom level for detailed view

try:
    # Search listings
    print("Searching for listings in V&A Waterfront...")
    ...
    # Save raw data
    with open('airbnb_waterfront_limited.json', 'w', encoding='utf-8') as f:
        json.dump(limited_results, f, indent=2, ensure_ascii=False)
    print("Raw results saved to airbnb_waterfront_limited.json")
