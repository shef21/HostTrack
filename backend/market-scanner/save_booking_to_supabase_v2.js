    ...
    try {
        ...
        for (const listing of rawData) {
            try {
                ...
                let propertyType = 'apartment'; // Default to apartment
                ...
                // Note: Hotels will be saved as apartments since 'hotel' is not in our enum

                // Generate a unique external ID for Booking.com listings
                const externalId = `booking_${Buffer.from(listing.url).toString('base64').slice(0, 32)}`;

                // Normalize rating to 0-5 scale (Booking.com uses 0-10)
                ...
                const formattedListing = {
                    ...
                    availability: {}, // Default empty object as per schema
