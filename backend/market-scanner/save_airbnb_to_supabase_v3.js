        ...
        for (const listing of rawData) {
            try {
                ...
                const formattedListing = {
                    area: 'Sea Point',
                    'property-type': listing.title.includes('Studio') ? 'studio' : 'apartment',
                    'external-id': `airbnb_${listing.room_id}`,
                    platform: 'airbnb',
                    title: listing.name,
                    'current-price': nightlyPrice,
                    'price-type': 'nightly',
                    bedrooms: bedrooms,
                    bathrooms: 1, // Default as Airbnb doesn't always provide this
                    'max-guests': 2, // Default
                    rating: listing.rating?.value || null,
                    'review-count': listing.rating?.reviewCount ? parseInt(listing.rating.reviewCount) : null,
                    'location-score': null,
